...
def gen_cropboxes(orig_image_dir, out_dir, time_min=0, time_max=-1, plot=True,
    ...
    for im in images[time_min:time_max]:
        a = read_image(str(im))
    ...
    hpairs['all'] = []
    for index, pair in enumerate(vpairs['all']):
        try:
            ...
            if plot:
                ...
                plt.plot(sm/max(sm))
                plt.axhline(threshold_after_filter)
                if os.path.exists(os.path.join(out_dir,'crop_' + str(index) + '.png')):
                    os.remove(os.path.join(out_dir,'crop_' + str(index) + '.png'))
                plt.savefig(os.path.join(out_dir,'crop_' + str(index) + '.png'))
    ...
    # Generate the MIP of the cropped images
    if gen_viz and num_boxes['all'] > 0:
        for im in images[time_min:time_max]:
            visualize_crops(str(im), out_dir)

    return num_boxes['all']


def visualize_crops(image_file: str, crop_dir: str):
    try:
...
def generate_crops(image_dir: str, crop_dir: str, output_dir: str,
    ...
    try:
        ...
        for im in images[time_min:time_max]:
            image_file = str(im)
            a = read_image(image_file)
            cur_box = a[:, crop_x_min:crop_x_max, crop_y_min:crop_y_max]
